// MEU-APP-CHAMADOS-FRONTEND/src/components/chamado-detail/Fase2Chamado.jsx
  ...
  return (
    <Box sx={{ pb: '100px' }}>
      ...
      <TabContext value={tabValue}>
        {emAndamento && (
          <Box sx={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: { xs: 0.5, sm: 1 }, mx: { xs: 1, sm: 3 }, mb: 2 }}>
          ...
          <TabPanel value="1" sx={{ p: 0 }}>
            <Paper elevation={4} sx={{ borderRadius: { xs: 0, sm: 3 }, overflow: 'hidden', p: 0 }}>
              <Box sx={{ textAlign: 'center', p: { xs: 2, sm: 3 }, background: `linear-gradient(145deg, ${theme.palette.primary.main}, ${theme.palette.secondary.main})`, color: 'white' }}>
                <Typography variant="h4" fontWeight="bold">{trabalho.id}</Typography>
                <Typography variant="h5">{`[${trabalho.ic}] ${trabalho.cliente}`}</Typography>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mt: 2, opacity: 0.9 }}><MapIcon sx={{ mr: 1 }} /><Typography>{trabalho.endereco}</Typography></Box>
              </Box>
              <Box sx={{ p: { xs: 2, sm: 3 }, display: 'flex', flexDirection: 'column', gap: 2 }}>
                {podeIniciarDeslocamento && (<GradientButton size="large" fullWidth onClick={props.handleIniciarDeslocamento} disabled={isUpdating}>Iniciar Deslocamento</GradientButton>)}
                {emDeslocamento && (<><Paper variant="outlined" sx={{ p: 2, textAlign: 'center' }}><Typography variant="overline">Tempo de Deslocamento</Typography><Cronometro dataInicio={trabalho.inicio_deslocamento} /></Paper><GradientButton fullWidth onClick={handleAbrirAprDialog} disabled={isUpdating}>Cheguei / Preencher APR</GradientButton></>)}
                {chegouAoLocal && (<><Button variant="contained" fullWidth color={trabalho.aprCompleta ? "success" : "warning"} onClick={() => setAprDialogOpen(true)}>{trabalho.aprCompleta ? "APR Preenchida" : "Preencher APR"}</Button><GradientButton size="large" fullWidth onClick={handleIniciarServico} disabled={!trabalho.aprCompleta || isUpdating}>Iniciar Serviço</GradientButton></>)}
                {emAndamento && (<Paper variant="outlined" sx={{ p: 2, textAlign: 'center' }}><Typography variant="overline">Tempo de Atendimento</Typography><Cronometro dataInicio={trabalho.inicio_atendimento} /></Paper>)}
              </Box>
            </Paper>
          </TabPanel>
          ...
          {emAndamento && (
            <Suspense fallback={<Box sx={{ display: 'flex', justifyContent: 'center', p: 4 }}><CircularProgress /></Box>}>
              <TabPanel value="2" sx={{ p: 0 }}>
                <Paper elevation={3} sx={{ p: 3, borderRadius: { xs: 0, sm: 3 }, textAlign: 'center' }}>
                  <Typography variant="h6">Descrição</Typography>
                  <Typography sx={{ whiteSpace: 'pre-wrap', mt: 2 }}>{trabalho.descricao}</Typography>
                </Paper>
              </TabPanel>
      ...
      {emAndamento && tabValue !== '5' && (
        <Paper 
          elevation={4} 
          sx={{ 
            position: 'sticky', bottom: 0, zIndex: 1100,
            background: 'rgba(255, 255, 255, 0.8)', // Fundo semitransparente para o rodapé
